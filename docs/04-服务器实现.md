...
private:
    ...
    std::vector<std::unique_ptr<Session>> sessions; // 会话列表
...
**关键方法实现:**
- [`create_and_start_session()`](../server/session_manager.cpp:9): 创建新会话并启动处理线程
- [`get_database()`](../server/session_manager.cpp:26): 返回数据库实例指针
...
**核心处理逻辑:**
- [`handle_session()`](../server/session.cpp:7): 主会话处理循环
- [`handle_login()`](../server/session.cpp:52): 处理用户登录
- [`handle_register()`](../server/session.cpp:69): 处理用户注册
...
#### 主处理循环 ([`session.cpp:7-50`](../server/session.cpp:7))
```cpp
void Session::handle_session() {
    while (true) {
        ...
        case ClientMessage::LOGIN:
            ...
            response.request = ServerMessage::LOGIN_RESPONSE;
        ...
        case ClientMessage::REGISTER:
            ...
            ServerPacket response;
            response.request = ServerMessage::REGISTER_RESPONSE;
            response.status = status;
            send_packet(socket, response);
            break;
...
#### 核心方法
- [`exist_user()`](../server/database.cpp:56): 检查用户是否存在
- [`add_user()`](../server/database.cpp:74): 添加新用户
- [`get_username()`](../server/database.cpp:93): 获取用户名
- [`get_password_hash()`](../server/database.cpp:121): 获取密码哈希
...
1. **会话管理**: [`close_session()`](../server/session_manager.cpp:20) 方法目前为空实现
2. **消息处理**: 目前只记录日志，需要实现实际的消息转发逻辑
3. **错误处理**: 需要更完善的异常处理和连接状态管理
4. **数据库方法**: [`database.hpp`](../server/database.hpp) 中声明的方法在 [`database.cpp`](../server/database.cpp) 中未完全实现
5. **协议一致性**: 服务器响应协议中存在重复的枚举值定义
